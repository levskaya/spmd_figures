...
export class Rect {
  ...
  clone(hide=true, reveal_time=null){
      ...
      const box = new Rect(this.position, this.size, this.color, this.opacity, scene, this.animations);
...
export class Box {
  ...
  get size() {
      return new THREE.Vector3(this.mesh.geometry.parameters.width, 
  ...
  toSize(size, t) {
      const new_geometry = new THREE.BoxGeometry( size.x, size.y, size.z ); 
      this.timeline.to(
        this.mesh.geometry.attributes.position.array,
        {endArray: new_geometry.attributes.position.array,
         onUpdate: () => this.mesh.geometry.attributes.position.needsUpdate = true},
         t)
      this.timeline.to(this.geometry.parameters, 
                       new_geometry.parameters, t);
      return this;
    }
}


// WebGL Text / Font handling.
