...
let t2 = push_packets(Y0s, y0_hbm, y0_vmem, t0 + hbm_tick*N, hbm_tick);
let t3 = push_packets(X1s, x1_hbm, x1_vmem, t2, hbm_tick);
let t4 = push_packets(Y1s, y1_hbm, y1_vmem, t2 + hbm_tick*N, hbm_tick);

let t5 = push_packets(X0s, x0_vmem, x_vreg, t2 + 3*hbm_tick, vmem_tick);
let t6 = push_packets(Y0s, y0_vmem, y_vreg, t2 + 3*hbm_tick, vmem_tick);
let t7 = push_packets(X0s, x_vreg, vector_core, t2 + 6*hbm_tick, vpu_tick);
         push_packets(Y0s, y_vreg, vector_core, t2 + 6*hbm_tick, vpu_tick);
...
let t9 = push_packets(Z0s, z_vreg, z0_vmem, t8, vmem_tick);
let t10 = push_packets(Z0s, z0_vmem, z0_hbm, t9, hbm_tick);

let t11 = push_packets(X1s, x1_vmem, x_vreg, t4 + 3*hbm_tick, vmem_tick);
let t12 = push_packets(Y1s, y1_vmem, y_vreg, t4 + 3*hbm_tick, vmem_tick);
...
let t15 = push_packets(Z1s, z_vreg, z1_vmem, t14, vmem_tick);
let t16 = push_packets(Z1s, z1_vmem, z1_hbm, t15, hbm_tick);
